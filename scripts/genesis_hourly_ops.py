...
def sync_github_issues(operations_report: dict) -> dict:
    ...
    for item in operations_report.get("issues", []):
        ...
        body = (
            f"{marker}\n"
            f"Automated Genesis operational issue.\n\n"
            ...
            "Genesis may attempt a repair only through its bounded task → candidate → test → Security → validator path."
        )
        current = by_key.get(key)
        desired_state = "closed" if item.get("status") == "resolved" else "open"
        if current is None and desired_state == "open":
            created_issue = _github_request("POST", "/issues", {
                "title": f"[Genesis Ops] {item.get('title')}",
                "body": body,
                "labels": ["genesis-ops"] if any("genesis-ops" == x.get("name") for x in []) else [],
            })
            if created_issue:
...
def render_email() -> tuple[str, str]:
    ...
    ops = GenesisOperations(ROOT).report()
    queue = PersistentTaskQueue(RUNTIME / "genesis_tasks.sqlite3")
    states = {state: len(queue.list(state=state, limit=1000)) for state in ("new", "assigned", "running", "blocked", "review", "complete", "failed")}
    ...
    lines = [
        ...
        "Generated and sent by Genesis from GitHub Actions.",
        "",
        "KPI DASHBOARD",
    ...
    if open_items:
        for item in open_items:
            lines.append(f"- [{item.get('severity','?').upper()}] {item.get('title')} | {item.get('status')} | module={item.get('module_id')} | {item.get('evidence')}")
    ...
    lines += [
        ...
        "Genesis queues solvable detected issues into its persistent engineering task system and may attempt one bounded repair per cycle. No issue may bypass tests, Security review, independent validation, protected files, signing boundaries, or owner-only secrets.",
    ]
    return subject, "\n".join(lines) + "\n"
...
def send_email() -> dict:
    ...
    recipient = os.environ.get("GENESIS_EMAIL_TO", "").strip()
    ...
    if not sender or not recipient or not username or not password:
        result = {
            "status": "blocked",
            "reason": "Missing GitHub mail secrets",
            "required": ["GENESIS_EMAIL_FROM", "GENESIS_EMAIL_TO", "GENESIS_SMTP_USERNAME", "GENESIS_SMTP_APP_PASSWORD"],
        }
...
def main() -> None:
    ...
    parser.add_argument("action", choices=("collect", "sync-issues", "email", "all"))
    args = parser.parse_args()

    if args.action in {"collect", "all"}:
        report = collect()
        print(json.dumps({"detected": len(report.get("issues", [])), "created_tasks": report.get("created_tasks", [])}, sort_keys=True))
