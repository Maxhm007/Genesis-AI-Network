...
BASELINE: tuple[CapabilityGap, ...] = (
    CapabilityGap("computer_use", "Computer-use interaction", "Observe and act on graphical interfaces through a bounded, verifiable computer-use loop.", ("genesis/computer_use.py", "genesis/gui_automation.py", "genesis/desktop_control.py"), ("computer_use", "gui_automation", "desktop_control"), "https://github.com/microsoft/autogen"),
    CapabilityGap("structured_output_schema", "Schema-constrained structured output", "Generate and validate machine-readable model output against an explicit schema.", ("genesis/structured_output.py", "genesis/schema_output.py"), ("structured_output", "json_schema", "schema_constrained_output"), "https://platform.openai.com/docs/guides/structured-outputs"),
    CapabilityGap("tool_call_planning", "Multi-step tool-call planning", "Plan, execute, verify, and revise bounded sequences of tool calls.", ("genesis/tool_planner.py", "genesis/tool_execution.py"), ("tool_call_planning", "tool_planner", "multi_step_tool_use"), "https://github.com/langchain-ai/langgraph"),
    CapabilityGap("retrieval_grounding", "Retrieval-grounded generation", "Ground model output in retrieved evidence with provenance and reject unsupported claims.", ("genesis/retrieval.py", "genesis/rag.py", "genesis/retrieval_grounding.py"), ("retrieval_grounding", "grounded_retrieval", "rag"), "https://github.com/huggingface/transformers"),
    CapabilityGap("context_compaction", "Long-context compaction", "Compact older context while preserving task-critical state for long-running agents.", ("genesis/context_compaction.py", "genesis/context_memory.py"), ("context_compaction", "context_compression", "long_context_compaction"), "https://github.com/vllm-project/vllm"),
    CapabilityGap("multimodal_vision", "Image understanding", "Accept image inputs and expose grounded visual observations to Genesis reasoning and tools.", ("genesis/vision.py", "genesis/multimodal.py", "genesis/image_understanding.py"), ("multimodal_vision", "image_understanding", "vision_input"), "https://github.com/huggingface/transformers"),
    CapabilityGap("audio_understanding", "Audio and speech understanding", "Process bounded audio or speech inputs into structured evidence usable by Genesis.", ("genesis/audio.py", "genesis/speech.py", "genesis/audio_understanding.py"), ("audio_understanding", "speech_recognition", "audio_input"), "https://github.com/huggingface/transformers"),
    CapabilityGap("persistent_memory", "Persistent agent memory", "Store and retrieve bounded long-lived task knowledge across runs with provenance and lifecycle controls.", ("genesis/persistent_memory.py", "genesis/memory_store.py"), ("persistent_memory", "long_term_memory", "memory_store"), "https://github.com/microsoft/autogen"),
)
...
def _request_json(url: str, token: str) -> object:
    request = urllib.request.Request(url, headers={"Authorization": f"Bearer {token}", "Accept": "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28", "User-Agent": "Genesis-AI-Network/missing-capability-discovery"})
    with urllib.request.urlopen(request, timeout=30) as response:
        return json.loads(response.read().decode("utf-8"))


def _post_json(url: str, token: str, payload: dict) -> object:
    request = urllib.request.Request(url, data=json.dumps(payload).encode("utf-8"), method="POST", headers={"Authorization": f"Bearer {token}", "Accept": "application/vnd.github+json", "Content-Type": "application/json", "X-GitHub-Api-Version": "2022-11-28", "User-Agent": "Genesis-AI-Network/missing-capability-discovery"})
    with urllib.request.urlopen(request, timeout=30) as response:
        return json.loads(response.read().decode("utf-8"))
...
def all_issue_texts(repo: str, token: str) -> list[str]:
    ...
    for page in range(1, 11):
        url = f"https://api.github.com/repos/{repo}/issues?" + urllib.parse.urlencode({"state": "all", "per_page": 100, "page": page})
...
def issue_payload(gap: CapabilityGap) -> dict:
    return {
        ...
        "body": issue_body(gap),
        "labels": ["genesis-task", "genesis-capability-discovery", "genesis-missing-capability", "genesis-autonomous", "agentic-lab"],
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Open at most one issue for one missing Qwen/Genesis capability and route it directly to Agentic Lab.")
    ...
    if not repo:
        print(json.dumps({"status": "no_repository"}, sort_keys=True)); return 0
    if not token and not args.dry_run:
        print(json.dumps({"status": "no_token"}, sort_keys=True)); return 0
    issues = all_issue_texts(repo, token) if token else []
    gap = choose_missing(ROOT, registered_capability_names(), issues)
    if gap is None:
        print(json.dumps({"status": "no_missing_capability"}, sort_keys=True)); return 0
    payload = {"status": "missing_capability_found", "capability_id": gap.capability_id, "title": gap.title, "fingerprint": fingerprint(gap.capability_id)}
    if args.dry_run:
