...
def put_registry(keys: dict[str, str]) -> None:
    ...
    args = [
        ...
        "PUT",
        "-f",
        "message=Pin persistent GDEN validator public keys",
        "-f",
        f"content={encoded}",
    ]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Provision persistent Ed25519 identities for GDEN validator nodes. "
            "Private keys are generated in memory and sent directly to GitHub Actions secrets via stdin."
        )
    )
    ...
    if not args.dry_run:
        print("Persistent keys provisioned. Node attestations have been dispatched; cryptographic consensus will activate after both signed attestations are published and the next consensus evaluation runs.")
    return 0
