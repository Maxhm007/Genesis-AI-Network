...
'''
    ...
    def _detected_exact_expression_provider(
        ...
        This path is intentionally narrow: the issue must be authored by GitHub Actions,
        use the Genesis detector title envelope, name one non-protected genesis Python target,
        and provide an exact expected expression in backticks. The expression identifiers must
        identify exactly one existing ``return`` statement. Normal proposal validation and the
        downstream full test suite remain authoritative.
        """
