...
def test_generate_identity_is_ed25519_raw_public_key() -> None:
    ...
    assert identity.private_pem.startswith("-----BEGIN PRIVATE KEY-----")
    assert len(base64.b64decode(identity.public_key_b64, validate=True)) == 32


def test_dry_run_never_requires_github_or_returns_private_material(monkeypatch) -> None:
    ...
    assert all("PRIVATE KEY" not in value for value in registry.values())


def test_private_key_is_sent_to_secret_command_over_stdin(monkeypatch) -> None:
    calls = []
