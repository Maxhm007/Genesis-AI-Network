...
def test_velocity_on_target_counts_only_validated_updates(tmp_path: Path) -> None:
    ...
    stamps = [now - timedelta(hours=6), now - timedelta(hours=4), now - timedelta(hours=2), now - timedelta(minutes=30)]
...
def test_velocity_does_not_reward_missing_validation_evidence(tmp_path: Path) -> None:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        json.dumps({"payload_type": "raw_commit", "timestamp": now.isoformat()}) + "\n",
        encoding="utf-8",
    )
