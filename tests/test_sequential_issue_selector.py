...
def test_urgent_action_failure_gets_lane_bonus():
    ...
    general = issue(11, 'General repair', 'Target exactly genesis/issue_lifecycle.py')
...
def test_high_value_newer_issue_can_beat_older_fifo_issue():
    older = issue(20, 'General repair', 'Target exactly genesis/issue_lifecycle.py', created_at='2026-09-01T00:00:00Z')
    newer = issue(21, '[Genesis Repair] critical blocker', 'Target exactly genesis/issue_lifecycle.py\nBlocks #30\nBlocks #31\nBlocks #32\nBlocks #33', labels=('severity-high', 'owner-priority', 'genesis-repair'), created_at='2026-09-18T00:00:00Z')
...
def test_stuck_issue_gets_retry_penalty():
    clean = issue(30, 'General repair', 'Target exactly genesis/issue_lifecycle.py')
    stuck = issue(31, 'General repair', 'Target exactly genesis/issue_lifecycle.py', labels=('genesis-stuck-10x',))
    assert candidate(clean, now=NOW)['score'] > candidate(stuck, now=NOW)['score']


def test_verified_or_blocked_issues_are_excluded():
    verified = issue(40, 'General repair', 'Target exactly genesis/issue_lifecycle.py', labels=('genesis-verified',))
    blocked = issue(41, 'General repair', 'Target exactly genesis/issue_lifecycle.py', labels=('genesis-blocked',))
    assert candidate(verified, now=NOW) is None
    assert candidate(blocked, now=NOW) is None


def test_selector_is_explainable():
    row = candidate(issue(50, '[Genesis Repair] reliability issue', 'Target exactly genesis/issue_lifecycle.py\nUnlocks #60', labels=('genesis-repair',)), now=NOW)
