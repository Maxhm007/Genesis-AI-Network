...
def test_gene_pulse_runtime_and_stale_cleanup_are_bounded() -> None:
    ...
    assert "GENESIS_PROVIDER_NAME: qwen3-0.6b-genesis-core" not in pulse
    assert "Qwen/Qwen3-0.6B" not in pulse
    assert "Restore Qwen cognitive caches" not in pulse
    assert "Install Qwen cognitive runtime" not in pulse
    assert "Start Genesis Qwen cognitive provider" not in pulse
    assert "GENESIS_PROVIDER_TIMEOUT_SECONDS" not in pulse
    assert "pip install torch" not in pulse
    assert "/tmp/genesis-qwen-core.log" not in pulse
